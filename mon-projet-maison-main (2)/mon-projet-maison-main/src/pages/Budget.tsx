...
const Budget = () => {
  ...
  const saveBudgetMutation = useMutation({
    mutationFn: async (categoriesToSave: BudgetCategory[]) => {
      ...
      // Update project total budget (with user_id check for security)
  ...
  const resetBudgetMutation = useMutation({
    mutationFn: async (options: { deletePlans?: boolean } = {}) => {
      ...
      // Reset project total budget to 0 (with user_id check for security)
      const { error: updateError } = await supabase
        .from("projects")
        ...
        <div className="container space-y-8">
            ...
            <div className="flex gap-2 flex-wrap">
              {selectedProjectId && hasAnalyzedBudget && (
                ...
                >
                  ...
                  {resetBudgetMutation.isPending ? t("budget.resetting") : t("budget.reset")}
                </Button>
              )}
              {selectedProjectId && (
                <Button 
                ...
                >
                  ...
                  {saveBudgetMutation.isPending ? t("budget.saving") : t("budget.save")}
                </Button>
              )}
              ...
              <CardContent>
                {projects.length > 0 ? (
                  ...
                  <p className="text-muted-foreground">
                    {t("budget.noProjectYet")} <a href="/start" className="text-primary underline">{t("budget.createOne")}</a>{t("budget.toSaveBudget")}
                  </p>
                )}
              </CardContent>
            </Card>
              ...
              <CardContent>
                {hasAnalyzedBudget ? (
                  <>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{t("budget.rangePercent")}</p>
                  </>
                ) : (
                  <>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card className="animate-fade-in" style={{ animationDelay: "300ms" }}>
              <CardHeader>
                <CardTitle className="font-display">{t("budget.budgetBreakdown")}</CardTitle>
                <CardDescription>{t("budget.overviewByCategory")}</CardDescription>
              </CardHeader>
                            ...
                            <div className="px-4 pb-3 pt-3 border-t bg-muted/30 space-y-4">
                              ...
                              {showAnalysisSummary && (
                                <div className="text-sm text-muted-foreground">
                                  <span className="font-medium">{t("budget.analysis")}</span> {category.description}
                                </div>
                              )}
                            </div>
                          </CollapsibleContent>
                      ...
                      <div className="rounded-lg border bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800 p-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-blue-700 dark:text-blue-400">{t("budget.taxes")}</span>
                          </div>
                          <div className="text-right shrink-0">
